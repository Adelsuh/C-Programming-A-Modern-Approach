...
/* prototypes */
void read_cards(int num_rank[], int num_suit[]);
void analyze_hand(int num_rank[], int num_suit[], bool *straight, bool *flush, bool *four,
                  ...
                  bool *three, int *pairs)
{
  ...
  *pairs = 0;
  ...
  /* check for flush */
  for (suit = 0; suit < NUM_SUITS; suit++)
    if (num_suit[suit] == NUM_CARDS)
      *flush = true;

  /* check for straight */
  rank = 0;
  while (num_rank[rank] == 0) rank++;
  for (; rank < NUM_RANKS && num_rank[rank] > 0; rank++)
    num_consec++;
